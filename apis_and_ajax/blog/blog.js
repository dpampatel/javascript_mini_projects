...
$(document).ready(async () => {
  try {
    // blog post id arrary
    const l_blog_ids = [2, 12, 22, 32, 42];
    // Promise object arrary
    const l_blog_promises = l_blog_ids.map(
      (id) => getObject(`${domain}/posts/${id}`) // returning promise object
    );
    const posts = await Promise.all(l_blog_promises); // Firing all calls
